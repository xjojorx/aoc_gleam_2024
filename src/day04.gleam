...
fn part1(input: String) -> Int {
  ...
  // explore(i, j, matrix)
  let horizontal = list.fold(matrix, 0, fn(acc, row) {
    let wins = list.window(row, 4)
    let n = list.count(wins, fn(w) {
      case w {
        ["X", "M", "A", "S"] -> True
        ["S", "A", "M", "X"] -> True
        _ -> False
  ...
  // io.debug(horizontal)
  let vertical = list.fold(list.transpose(matrix), 0, fn(acc, row) {
    let wins = list.window(row, 4)
    let n = list.count(wins, fn(w) {
      case w {
        ["X", "M", "A", "S"] -> True
  ...
  let rowset = list.window(matrix, 4)
...
fn to_square_windows(matrix: List(List(a)), size: Int) {
  let rowset = list.window(matrix, size)
