...
fn do_get_antinodes(
  ...
  case antennas {
    ...
    [ant, ..rest] -> {
      let new_set =
        list.fold(rest, acc, fn(set_acc, x) {
...
fn get_antinodes2(map: Map, antenna: String) -> Set(Position) {
  let assert Ok(positions) = dict.get(map.antennas, antenna)
  let start = set.from_list(positions)
  // now all antennas are antinodes
  do_get_antinodes2(positions, map.width, map.height, start)
}

fn do_get_antinodes2(
  antennas: List(Position),
  width: Int,
  height: Int,
  acc: Set(Position),
...
fn antinodes_for_pair(
  ...
  yielder.unfold(1, fn(n) {
    let antinode_fwd =
      Position(a2.row + { delta_row * n }, a2.col + { delta_col * n })
    let antinode_bck =
      Position(a1.row - { delta_row * n }, a1.col - { delta_col * n })

    let val =
      [antinode_fwd, antinode_bck]
      |> list.filter(position_in_map(_, width, height))
